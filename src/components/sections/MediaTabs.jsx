    ...
    <section id="media" className="py-12 sm:py-16 lg:py-20 bg-white">
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="flex justify-center mb-8 sm:mb-12">
          <div className="bg-gray-100 rounded-full p-1 sm:p-2 flex space-x-reverse space-x-1 sm:space-x-2 overflow-x-auto">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`flex items-center space-x-reverse space-x-2 px-4 sm:px-6 py-2 sm:py-3 rounded-full font-medium transition-all duration-300 whitespace-nowrap text-sm sm:text-base ${
                  activeTab === tab.id
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <div className="absolute bottom-4 right-4 left-4 text-white">
                        <h3 className="font-bold mb-1 text-sm sm:text-base">{image.title}</h3>
                ...
                {mediaData.videos.map((video, index) => (
                  ...
                  >
                    ...
                    <div className="p-4 sm:p-6">
                      <h3 className="font-bold text-gray-800 mb-2 text-sm sm:text-base">{video.title}</h3>
                  ...
                  >
                    <div className="flex items-start space-x-reverse space-x-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-bold text-gray-800 mb-2 text-sm sm:text-base">{doc.title}</h3>
                        <div className="flex items-center space-x-reverse space-x-4 text-xs sm:text-sm text-gray-500 mb-4">
                          <span>{doc.type}</span>
                          <span>{doc.size}</span>
                        </div>
                        <div className="flex items-center justify-between">
                          ...
                          <button className="bg-brand text-white px-3 sm:px-4 py-2 rounded-lg hover:bg-brand-dark transition-colors text-xs sm:text-sm">
                            تحميل
                          </button>
                        </div>
                      </div>
